...
|:---:|:---:|:---|:---|
| PM | 이주헌 | 계획·복구 엔진 및 백엔드 통합 | 전체 백엔드 구조 설계, planner 모델, 예상시간 계산, 실현 가능성 판정, 날짜별 일정 생성, 분량 유지형·핵심 집중형 복구 계산, 복구안 적용, 백엔드 기능 통합 |
...
| BE3 | 김유겸 | AI 분석·진행 기록·속도 보정·배포 | 추출된 텍스트를 AI에 전달, 단원·학습 작업 생성, 작업 유형·중요도·깊이·난이도 추천, AI 응답 JSON 검증·예외처리, 공부 진행 결과 저장, 실제 공부 속도 계산 및 speed_factor 보정, GCP 배포 및 인프라 구성 |
