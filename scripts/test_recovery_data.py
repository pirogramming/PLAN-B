...
def main():
    # scripts/에서 직접 실행해도 프로젝트 모듈을 찾을 수 있도록 루트 경로 추가
    ...
    for order, task in enumerate(
        [task_core, task_optional1, task_optional2, task_basic],
        start=1,
    ):
        items.append(
            DailyPlanItem.objects.create(
                ...
                study_task=task,
                planned_minutes=task.estimated_max_minutes,
                order=order,
            )
        )

    ProgressLog.objects.create(
        daily_plan_item=items[0],
    ...
    if recovery_plans:
        ...
        print(
            f"확인 URL: "
            f"http://127.0.0.1:8000/planner/recovery/{group_id}/"
        )
    else:
        print(
            "recovery_plans가 비어있습니다 - "
            "복구안이 생성되지 않았습니다."
        )
