...
class TaskReviewFormSubmitTest(TestCase):
    ...
    def test_confirm_and_next_action(self):
        """'모두 확정하고 다음으로' 버튼 클릭 시 실제 planner:feasibility 라우팅 검증 (모킹 제거 반영)"""
        post_data = {
            "form-TOTAL_FORMS": "1",
            "form-INITIAL_FORMS": "1",
            "form-MIN_NUM_FORMS": "0",
        ...
        # 리다이렉트 URL 및 kwargs 인자 정상 전달 검증
...
class MaterialAnalysisViewTestCase(TestCase):
    ...
    # =========================================================================
    # [리뷰어 요청 반영] 타임아웃 응답 검증 통합 테스트
    # =========================================================================
    def test_analysis_status_returns_failed_when_orchestrator_detects_timeout(self):
        """오래된 PROCESSING 상태 조회 시 get_analysis_status에서 반환된 타임아웃(FAILED) 상태가 API에 반영되는지 검증"""
        ...
        with patch("exams.views.get_analysis_status", return_value=mock_timeout_data):
            response = self.client.get(reverse('exams:material_analysis_status', args=[self.material.id]))
            self.assertEqual(response.status_code, 200)
            data = response.json()
            ...
            self.assertEqual(data["analysis_error_message"], "분석 시간이 초과되었습니다.")
            self.assertEqual(data["retry_count"], 0)
            self.assertEqual(data["retry_remaining"], 2)
