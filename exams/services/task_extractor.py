...
4. 각 학습 작업은 일반적으로 20~60분 안에 완료할 수 있는 크기를 목표로 한다
   ...
   b. 아래 조건 중 하나라도 원문에 명시적으로 나타나면, 그 조건에 맞춰서만
      ...
      독립된 이름과 목적을 가진 예제들), 이것만으로는 작업을 분리하는 근거가
...
8. 입력 텍스트 안에 "--- 페이지 N ---" 형식의 표시가 있다면, 이는 PDF에서
   ...
   - 단, 페이지를 빠뜨리지 않으려고 서로 다른 학습 목표를 가진 내용을
     ...
     다만 규칙 4에서 "서로 다른 이름이 붙은 응용 예제는 항상 하나의 작업으로
     합친다"고 이미 정했으므로, 이 경우엔 그 병합된 작업의 source_pages에
     핵심 알고리즘 개념/구현 페이지와 응용 예제(다항식 표현, 학생 정보 관리,
     음악 재생목록, 이미지 뷰어 등) 페이지를 전부 포함시켜라. 예제가 있다는
     이유로 그 페이지를 따로 빼거나 source_pages에서 누락시키지 마라.
