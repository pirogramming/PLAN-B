   ...
   b. 아래 조건 중 하나라도 원문에 명시적으로 나타나면, 그 조건에 맞춰서만
      ...
      - 개념 이해 / 비교·정리 / 계산·적용 / 문제 풀이 / 최종 복습처럼 서로
        ...
        → 학습 행동별로 분리하되, 위 조건(개념/구현 분리)으로 이미 나뉜
          ...
          (즉 같은 내용을 두 조건으로 중복 분리하지 않는다). 이 조건
          자체로 새로 만드는 작업은 최대 3개다(이미 concept/practice로
          나뉜 학습 행동은 이 3개에 포함하지 않는다). 원문에 명시된
          학습 행동이 그보다 많으면, 그만큼 더 쪼개지 말고 성격이 비슷한
          행동끼리 묶어서(예: "개념 이해"와 "비교·정리"는 함께 concept로,
          "계산·적용"과 "문제 풀이"는 함께 practice로) 3개 이하로 병합하라.
          "최종 복습"은 억지로 병합하지 말고 task_type=review로 별도
          유지해도 된다. 단, 원문에서 명확한 소제목/장 구분으로 나뉘는
          넓은 단원이라면 병합 대신 d를 적용해서 단원 자체를 먼저 나누고,
          나뉜 각 단원 안에서 이 조건을 다시 적용하라.
