   ...
   b. 아래 조건 중 하나라도 원문에 명시적으로 나타나면, 그 조건에 맞춰서만
      ...
      - 개념 이해 / 비교·정리 / 계산·적용 / 문제 풀이 / 최종 복습처럼 서로
        ...
        → 학습 행동별로 분리하되, 위 조건(개념/구현 분리)으로 이미 나뉜
          ...
          (즉 같은 내용을 두 조건으로 중복 분리하지 않는다). 이 조건으로
          한 단원에서 만들 수 있는 작업은 최대 2~3개로 제한한다. 그 이상
          세분화가 필요해 보인다면 작업을 더 쪼개지 말고 d를 적용해서
          단원 자체를 나눠라.
...
6. importance / depth / difficulty는 아래 기준표를 그대로 적용해서 정하라.
   ...
   다른 개념의 기반이 되는가"를 본다. 같은 신호(예: 선수 지식 언급)를
   두 필드에 똑같이 적용하지 마라. 아래 기준표에서 각 필드가 보는
   신호가 다르다는 것에 유의해서, 필드마다 원문에서 해당하는 신호가
   있는지 따로 확인하라.

   importance (중요도 - 시험에서의 우선순위 기준):
   - high: 원문에서 "핵심", "중요", "필수"라고 명시되었거나 강조된 경우
   - medium: 위 조건에 해당하지 않는 일반적인 시험 범위 내용 (기본값)
   - low: 원문에서 "참고", "부가", "선택"이라고 명시되어 중요도가 낮은 경우
   ...
   구분하라. high/basic(강조되지만 독립적인 내용)이나 medium/core(선수
   개념이지만 강조 표현은 없는 내용)처럼 두 필드가 다른 값을 갖는 조합도
   자연스러운 결과다.
