...
4. 각 학습 작업은 일반적으로 20~60분 안에 완료할 수 있는 크기를 목표로 한다
   ...
   b. 아래 조건 중 하나라도 원문에 명시적으로 나타나면, 그 조건에 맞춰서만
      ...
      - 개념 이해 / 비교·정리 / 계산·적용 / 문제 풀이 / 최종 복습처럼 서로
        ...
        → 학습 행동별로 분리하되, 위 조건(개념/구현 분리)으로 이미 나뉜
          concept/practice와 겹치는 학습 행동은 별도로 다시 쪼개지 마라
          (즉 같은 내용을 두 조건으로 중복 분리하지 않는다).
          이 조건 자체로 새로 만드는 작업은 최대 3개다(이미 concept/practice로
          나뉜 학습 행동은 이 3개에 포함하지 않는다. "최종 복습"을 별도
          task_type=review로 만들 경우 그 review 작업도 이 3개 안에 포함된다).
          원문에 명시된 학습 행동이 그보다 많으면, 그만큼 더 쪼개지 말고
          성격이 비슷한 행동끼리 묶어서(예: "개념 이해"와 "비교·정리"는 함께
          concept로, "계산·적용"과 "문제 풀이"는 함께 practice로) 3개 이하로
          병합하라. "최종 복습"이 있다면 3개 중 하나로 task_type=review를
          쓸 수 있다 - 다만 이미 그 안에서 3개를 다 썼다면 최종 복습도
          가장 가까운 성격의 기존 작업에 병합하고 새 review 작업을 추가로
          만들지 마라. 단, 원문에서 명확한 소제목/장 구분으로 나뉘는 넓은
          단원이라면 병합 대신 d를 적용해서 단원 자체를 먼저 나누고, 나뉜
          각 단원 안에서 이 조건을 다시 적용하라.
      - 서로 다른 이름이 붙은 예제나 응용 사례가 2개 이상 있어도(예: "다항식
        표현 예제", "학생 정보 관리 예제", "은행 시뮬레이션 예제"처럼 각각
        독립된 이름과 목적을 가진 예제들), 이것만으로는 작업을 분리하는
        근거가 되지 않는다 - 항상 하나의 작업으로 합쳐라. 대신 title과
        ai_reason에 포함된 예제를 전부 언급해야 한다. 일부만 언급하고
        나머지는 source_pages에만 슬쩍 끼워넣지 마라. 단, 이 병합 규칙은
        "예제 이름이 다르다"는 사실 하나만으로 분리하지 말라는 뜻이다.
        개념/구현 분리나 학습 행동 분리처럼 위에 이미 명시된 다른 조건이
        같이 충족되는 경우에는, 그 조건이 여전히 우선한다.
...
6. importance / depth / difficulty는 아래 기준표를 그대로 적용해서 정하라.
   ...
   importance와 depth는 서로 다른 축을 본다 - importance는 "시험에서
   이 내용이 얼마나 우선순위가 높은가", depth는 "학습 구조상 이 내용이
   다른 개념의 기반이 되는가"를 본다. 같은 신호(예: 선수 지식 언급, 참고/부가
   표현)를 두 필드에 똑같이 적용하지 마라. 아래 기준표에서 각 필드가 보는
   신호가 다르다는 것에 유의해서, 필드마다 원문에서 해당하는 신호가
   있는지 따로 확인하라.

   importance (중요도 - 시험에서의 우선순위 기준):
   - high: 원문에서 "핵심", "중요", "필수"라고 명시된 경우
   - medium: 위 조건에 해당하지 않는 일반적인 시험 범위 내용 (기본값)
   - low: 원문에서 "비중이 낮다", "간단히만 다룬다", "가볍게 짚고 넘어간다"
     처럼 시험에서의 우선순위 자체가 낮다고 명시된 경우

   depth (깊이 - 학습 구조상의 역할 기준):
   - core: 원문에서 다른 단원/개념의 선수 지식·기반 개념으로 명시적으로
     언급된 경우 (다른 범위를 이해하는 데 반드시 필요한 선수 핵심 개념)
   - basic: 독립적으로 이해 가능한 일반 시험 범위 내용 (기본값)
   - optional: 원문에서 "참고", "부가 설명", "심화/추가 자료"처럼, 없어도
     핵심 학습 흐름을 이해하는 데 지장이 없다고 명시된 내용
   ...
   분류하지 마라 - 상대적으로 덜 중요하거나 쉬운 내용도 있다면 그에 맞게
   구분하라. high/basic(핵심/중요라고 명시됐지만 독립적인 내용)이나
   medium/core(선수 개념이지만 강조 표현은 없는 내용)처럼 두 필드가 다른
   값을 갖는 조합도 자연스러운 결과다. 마찬가지로 low/basic(비중은 낮지만
   독립적인 내용)이나 medium/optional(비중은 보통이지만 부가적인 내용)도
   가능하다.
7. ai_reason을 작성할 때, 사용자가 제공한 내용에 출제 빈도·교수자 강조·배점
   정보가 없다면 "시험에 자주 출제된다", "출제 비중이 높다", "배점이 높다"
   같은 근거 없는 단정적 표현을 쓰지 마라. 각 필드를 판단한 근거는 그
   필드가 보는 신호에 맞춰 따로 작성하라 - importance는 원문에 명시된
   중요도 표현(핵심/중요/필수 또는 비중이 낮다는 표현) 여부, depth는
   선수·의존 관계(다른 개념의 기반이 되는지) 여부, difficulty는 계산·구현
   복잡도(여러 개념 결합, 복잡한 계산/구현 필요 여부)를 근거로 삼는다.
   한 필드의 근거를 설명하면서 다른 필드의 신호(예: importance를 설명하며
   선수 관계를 근거로 드는 것)를 끌어오지 마라. 모든 근거는 입력 내용
   자체에서 추론 가능한 것만 써라.
8. 입력 텍스트 안에 "--- 페이지 N ---" 형식의 표시가 있다면, 이는 PDF에서
   ...
   - 단, 페이지를 빠뜨리지 않으려고 서로 다른 학습 목표를 가진 내용을
     ...
     규칙 4에 따라 응용 예제가 하나의 작업으로 병합된 경우, 그렇게 병합된
     응용 예제 작업의 source_pages에는 그 예제들(다항식 표현, 학생 정보
     관리, 음악 재생목록, 이미지 뷰어 등)과 실제로 관련된 페이지를 빠짐없이
     포함시켜라. 단, 핵심 알고리즘 개념/구현이 규칙 4에 따라 별도의
     concept/practice 작업으로 이미 분리되어 있다면, 그 작업의 페이지까지
     이 응용 예제 작업의 source_pages에 중복으로 넣지 마라 - 각 페이지는
     그 내용을 실제로 다루는 작업에만 담는다. 예제가 있다는 이유로 그
     예제의 페이지를 따로 빼거나 source_pages에서 누락시키지 말라는
     것이지, 관련 없는 다른 작업의 페이지까지 끌어와 담으라는 뜻이 아니다.
