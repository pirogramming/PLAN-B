...
def _sort_key(task: TaskInput):
    ...
    시험일 -> 중요도까지는 "어떤 작업이 부족한 자리를 우선 차지할지"를
    결정하는 정책 순서라서 그대로 둔다.

    그 다음에 estimated_max_minutes 내림차순을 추가한 이유(Best-Fit Decreasing):
    같은 시험일·중요도 안에서 작은 작업부터 넣으면(First-Fit) 큰 작업이 들어갈
    자리가 없어 "총 가용시간은 충분한데 미배치가 발생하는" 공간 파편화가
    생긴다. 큰 작업을 먼저 배치하면 이 문제가 크게 줄어든다.

    order/id는 같은 시험일·중요도·소요시간 안에서의 최종 타이브레이커로만 쓴다.
    """
...
def allocate_tasks_to_days(
    ...
    배치 규칙:
        - 정렬: 시험일 오름차순 -> 중요도(high>medium>low)
                -> estimated_max_minutes 내림차순 -> order -> id
